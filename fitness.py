...
def get_fitness(gene_data):
    ...
    for i in range(170):
        ...
        for point_info in red_point_list[i].possible_point_list:
            ...
            # 파란점이 서비스 범위 밖이면 넘어감
            if distance > Cz_list[use_case]:
                continue

            # 현재 파란점이 이미 "use_case"시설에 대해 계산되었다면 넘어감
            if is_calculated_list[bluePoint.point_index - 1][use_case]:
                continue

            total_D += bluePoint.D_list[use_case]
            if expected_time != -1:
                expected_time_list.append(expected_time)

            is_calculated_list[bluePoint.point_index - 1][use_case] = True
